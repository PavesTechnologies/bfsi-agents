...
const Toast = React.forwardRef<React.ElementRef<typeof ToastPrimitive.Root>, React.ComponentPropsWithoutRef<typeof ToastPrimitive.Root> & { variant?: 'default' | 'destructive' }>(
  ({ className, variant = 'default', ...props }, ref) => (
    <ToastPrimitive.Root
      ref={ref}
      className={cn('group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md border p-6 pr-8 shadow-lg transition-all', variant === 'destructive' ? 'destructive border-destructive bg-destructive text-destructive-foreground' : 'border bg-background text-foreground', className)}
      {...props}
    />
  ),
)
...
interface ToastMessage { id: string; title: string; description?: string; variant?: 'default' | 'destructive' }
let toastListeners: Array<(toasts: ToastMessage[]) => void> = []
let toastQueue: ToastMessage[] = []

function notify(msg: Omit<ToastMessage, 'id'>) {
  const id = Math.random().toString(36).slice(2)
  toastQueue = [...toastQueue, { ...msg, id }]
...
export function toast(msg: Omit<ToastMessage, 'id'>) { notify(msg) }
