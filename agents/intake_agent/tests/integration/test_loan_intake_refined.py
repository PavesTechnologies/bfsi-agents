...
@pytest.fixture
def client():
    # 'with' triggers the lifespan events (DB initialization)
...
def test_tc_i1_success_full_payload(client):
...
def test_tc_i2_success_min_payload(client):
...
def test_tc_i3_fail_missing_required(client):
...
def test_tc_i4_fail_invalid_amount(client):
...
def test_tc_i5_fail_invalid_term(client):
...
def test_tc_i6_idempotency_duplicate(client):
    """TC-I6: Idempotency - Duplicate Submission"""
    payload = get_valid_payload()
    
    # First submission
    r1 = client.post("/loan_intake/submit_application", json=payload)
    assert r1.status_code == 200
    id1 = r1.json()["application_id"]
    
    # Second submission with same request_id
    r2 = client.post("/loan_intake/submit_application", json=payload)
    assert r2.status_code == 200
    id2 = r2.json()["application_id"]
    
    assert id1 == id2

def test_tc_i7_non_blocking_bad_email(client):
    """TC-I7: Non-Blocking - Bad Email"""
    payload = get_valid_payload()
    payload["applicants"][0]["email"] = "invalid-email"
...
def test_tc_i8_non_blocking_underage(client):
