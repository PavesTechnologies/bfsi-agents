...
from src.utils.validation.blocking_aggregator import validate_all_applicants_blocking

from vault.agent_services.source_code.email_service import EmailVaultService
from vault.agent_services.source_code.phone_number_service import (
    PhoneNumberVaultService,
)
from vault.agent_services.source_code.ssn_service import SSNVaultService


class LoanIntakeService:
    def __init__(self, db: AsyncSession, idempotency: IdempotencyGuard | None = None):
        self.db = db
    ...
    async def submit_application(
        self, request: LoanIntakeRequest
    ) -> LoanIntakeResponse:
        try:
            ...
            async def first_execution():
                ...
                for applicant in normalized_req.applicants:
                    print(
                        f"Validating applicant {applicant.first_name}\
                              {applicant.last_name} with non-blocking validators..."
                    )
                    summary = validate_applicant(applicant)
                    print(
                        f"Validation summary for applicant {applicant.first_name} \
                            {applicant.last_name}: {summary}"
                    )
                    ...
                    for res in summary.results:
                        ...
                        if not res.is_valid:
                            ...
                            # ❌ REMOVED: raise HTTPException(status_code=400,
                            # detail=validation_issues[-1]["message"])
                    ...
                    applicant_row = await self.dao.create_applicant(
                        {
                            ...
                            "ssn_encrypted": self.ssn_service.protect_ssn(
                                applicant.ssn_no
                            ),
                            ...
                            "phone_number": self.phone_number_service.protect_phone_number(  # noqa: E501
                    ...
                    if applicant.employment:
                        ...
                        employment_validations = [
                            (
                                "type",
                                validate_employment_type(employment.employment_type),
                            ),
                            (
                                "employer_name",
                                validate_employer_name(employment.employer_name),
                            ),
                    ...
                    for liability in applicant.liabilities:
                        ...
                        await self.dao.create_liability(
                            {"applicant_id": applicant_row.applicant_id, **data}
                        )
                ...
                await IdempotencyRepository(self.db).mark_completed(
                    request_id=request.request_id, response_payload=response_payload
                )

                return response_payload

            # 1. Blocking Validations
            # (Run BEFORE idempotency to avoid caching bad requests)
            validation_summary = validate_all_applicants_blocking(request)
            if not validation_summary.is_valid:
                raise HTTPException(
                    status_code=400, detail=validation_summary.to_http_detail()
                )
            ...
            raise HTTPException(
                status_code=500, detail=f"Database error occurred: {e}"
            ) from None

        except RuntimeError:
            raise HTTPException(
                status_code=409, detail="Request already in progress"
            ) from None
