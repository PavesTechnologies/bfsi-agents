...
def validate_applicant_blocking(applicant) -> BlockingValidationSummary:
    ...
    if not result.passed:
        errors.append(
            ValidationError(field="applicant.first_name", message=result.message)
        )
    ...
    if not result.passed:
        errors.append(
            ValidationError(field="applicant.last_name", message=result.message)
        )
    ...
    if not result.passed:
        errors.append(
            ValidationError(field="applicant.ssn_last4", message=result.message)
        )
    ...
    if ssn_no:
        ...
        if not result.passed:
            errors.append(
                ValidationError(field="applicant.ssn_no", message=result.message)
            )
    ...
    if not result.passed:
        ...
        if result.reason_code != ValidationReasonCode.AGE_BELOW_MINIMUM:
            errors.append(
                ValidationError(field="applicant.date_of_birth", message=result.message)
            )
    ...
    for idx, address in enumerate(addresses):
        ...
        if not result.passed:
            errors.append(
                ValidationError(
                    field=f"address[{idx}].address_line1", message=result.message
                )
            )
        ...
        if not result.passed:
            errors.append(
                ValidationError(field=f"address[{idx}].city", message=result.message)
            )
        ...
        if not result.passed:
            errors.append(
                ValidationError(field=f"address[{idx}].state", message=result.message)
            )
        ...
        if not result.passed:
            errors.append(
                ValidationError(
                    field=f"address[{idx}].zip_code", message=result.message
                )
            )
    ...
    if employment:
        ...
        if not result.passed:
            errors.append(
                ValidationError(
                    field="employment.employment_type", message=result.message
                )
            )
        ...
        if not result.passed:
            errors.append(
                ValidationError(
                    field="employment.employer_name", message=result.message
                )
            )
        ...
        if not result.passed:
            errors.append(
                ValidationError(field="employment.job_title", message=result.message)
            )
        ...
        if not result.passed:
            errors.append(
                ValidationError(
                    field="employment.gross_monthly_income", message=result.message
                )
            )
...
def validate_loan_blocking(loan) -> BlockingValidationSummary:
    ...
    if not result.passed:
        errors.append(
            ValidationError(field="requested_term_months", message=result.message)
        )
