...
class EmploymentInfo(BaseModel):
    ...
    employment_status: str = Field(..., description="Current employment status (e.g., active, inactive)")
    ...
    gross_monthly_income: float = Field(..., description="Gross monthly income in local currency")
    self_employed_flag: bool = Field(..., description="True if the applicant is self-employed")


class ApplicantInfo(BaseModel):
    """Applicant identity and contact information."""

    applicant_role: str = Field(..., description="Role of applicant in the loan (e.g., primary, co-applicant)")
    first_name: str = Field(..., description="Applicant's first name")
    middle_name: Optional[str] = Field(None, description="Applicant's middle name, if any")
    ...
    ssn_last4: Optional[str] = Field(None, description="Last 4 digits of SSN, if provided")
    itin_number: Optional[str] = Field(None, description="ITIN number, if provided")
    citizenship_status: str = Field(..., description="Citizenship status (e.g., citizen, resident)")
    email: str = Field(..., description="Applicant's contact email address")
    employment: EmploymentInfo = Field(..., description="Applicant's employment information")


class LoanIntakeRequest(BaseModel):
    """Top-level loan intake request model for submission endpoint."""

    request_id: uuid.UUID = Field(..., description="Unique identifier for the intake request")
    callback_url: Optional[AnyUrl] = Field(None, description="Optional callback URL to receive updates")
    app_id: uuid.UUID = Field(..., description="Application identifier")
    loan_type: str = Field(..., description="Type of loan requested (e.g., mortgage, personal)")
    ...
    applicants: List[ApplicantInfo] = Field(..., description="List of applicants associated with this request")
