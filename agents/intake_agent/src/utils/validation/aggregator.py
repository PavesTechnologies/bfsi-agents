...
def validate_applicant(applicant) -> ValidationSummary:
    ...
    try:
        ...
        try:
            ...
            results.append(ValidationResult(field="date_of_birth", is_valid=False, reason=f"validation error: {exc}"))
        ...
        failed_fields = list(dict.fromkeys([r.field for r in results if not r.is_valid]))
        is_valid = all(r.is_valid for r in results)

        return ValidationSummary(is_valid=is_valid, results=results, failed_fields=failed_fields)

    except Exception as exc:  # pragma: no cover - defensive
        # Catch any unforeseen errors and return them as a failing summary
        fallback = ValidationResult(field="applicant", is_valid=False, reason=f"aggregator error: {exc}")
        return ValidationSummary(is_valid=False, results=[fallback], failed_fields=["applicant"])