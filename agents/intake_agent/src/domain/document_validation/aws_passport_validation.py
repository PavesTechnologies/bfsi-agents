...
class PassportOCR:
    ...
    def is_passport_document(self, lines):
        ...
        keywords = ["PASSPORT", "UNITED STATES OF AMERICA", "USA"]
    ...
    def parse_mrz(self, lines):
        ...
        for line in lines:
            ...
            if re.fullmatch(
                r"[A-Z0-9<]{44,46}", line
            ):  # write one only accept 44 chars but we will normalize it to 44,
                # allow some extra chars for OCR errors
        ...
        dob = l2[13:19]
        dob_cd = l2[19]  # noqa: F841

        # if self.mrz_checksum(dob) != int(dob_cd):
        #     return None

        expiry = l2[21:27]
        exp_cd = l2[27]  # noqa: F841
    ...
    def process_file(self, user_file, document_type, application_id):
        ...
        try:
            confidence = self.calculate_confidence(lines=lines, mrz_data=mrz_data)
