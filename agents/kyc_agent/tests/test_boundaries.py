...
def test_leap_year_dob_processing(leap_year_dob_fixture):
    ...
    print(f"Vendor Response DOB: {vendor_response.consumerIdentity.dob}") # Debug log for vendor DOB parsing
...
def test_max_length_resilience(max_length_identity_fixture):
    ...
    assert vendor_response.consumerIdentity.name[0].firstName == \
