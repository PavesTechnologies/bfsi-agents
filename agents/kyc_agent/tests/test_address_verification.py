...
class TestAddressVerificationService:
    ...
    def test_golden_path_address_match(self):
        ...
        result = IdentityService.process_address_verification(
            GOLDEN_REQUEST, experian_data
        )
        ...
        print(
            f"""  ✅ Golden path: match={result["address_match"]},
    ...
    def test_mismatched_city(self):
        ...
        request = {
            **GOLDEN_REQUEST,
            "address": {**GOLDEN_REQUEST["address"], "city": "New York"},
        }
        experian_data = _get_experian_response(
            GOLDEN_REQUEST
        )  # Use original for adapter
        result = IdentityService.process_address_verification(request, experian_data)

        assert "CITY_MISMATCH" in result["flags"]
        assert result["risk_score"] > 0.0
        print(
            f"  ✅ City mismatch: flags={result['flags']}, risk={result['risk_score']}"
        )
    ...
    def test_mismatched_state(self):
        ...
        request = {
            **GOLDEN_REQUEST,
            "address": {**GOLDEN_REQUEST["address"], "state": "NY"},
        }
        experian_data = _get_experian_response(GOLDEN_REQUEST)
        result = IdentityService.process_address_verification(request, experian_data)
    ...
    def test_mismatched_zip(self):
        ...
        request = {
            **GOLDEN_REQUEST,
            "address": {**GOLDEN_REQUEST["address"], "zip": "99999"},
        }
        experian_data = _get_experian_response(GOLDEN_REQUEST)
        result = IdentityService.process_address_verification(request, experian_data)
    ...
    def test_completely_different_address_fails(self):
        ...
        print(
            f"""  ✅ Full mismatch: match={result["address_match"]},
             risk={result["risk_score"]}, flags={result["flags"]}"""
        )
