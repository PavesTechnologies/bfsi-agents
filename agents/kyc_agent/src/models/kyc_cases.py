...
class KYC(Base):
    ...
    applicant_id = mapped_column(UUID, nullable=False, index=True)

    payload_hash = mapped_column(String(64), nullable=False, index=True)

    # 🔹 NEW COLUMN
    raw_request_payload = mapped_column(JSONB, nullable=False)
    ...
    # Different KYC providers might have different rules and models,
    # so we snapshot them at the time of the attempt
    # EX: "kyc_v1"
    rules_version = mapped_column(String(50))
    model_versions = mapped_column(JSONB)  # model name -> version
    threshold_snapshot = mapped_column(
        JSONB
    )  # model name -> threshold used for pass/fail
    ...
    face_check = relationship(
        "FaceCheck", back_populates="kyc", uselist=False, cascade="all, delete-orphan"
    )

    aml_check = relationship(
        "AMLCheck", back_populates="kyc", uselist=False, cascade="all, delete-orphan"
    )
