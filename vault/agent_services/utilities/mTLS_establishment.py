...
import tempfile
import os
from agent_services.utilities.load_secrets import get_aws_secret
...
def establish_mtls_context():
    ...
    try:
        ...
        if os.path.exists(cert_f.name): os.unlink(cert_f.name)
        if os.path.exists(key_f.name): os.unlink(key_f.name)
        if os.path.exists(ca_f.name): os.unlink(ca_f.name)
